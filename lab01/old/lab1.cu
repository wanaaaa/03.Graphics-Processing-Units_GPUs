...
__global__ void testLoop(float * tempGround, unsigned int N);
...
void  gpu_heat_dist(float * playground, unsigned int N, unsigned int iterations)
{
   ...
   int threadNum = 64;
   int blockNum = (numElements + threadNum)/threadNum;
   ...
   for(int i = 0; i < N*N; i++) {
      cout<<"i-> "<<i <<" value->"<<  h_tempResult[i]<<endl;
   }
...
__global__ void testLoop(float * tempGround, unsigned int N) {
   ...
   // if(ix < N)
      for(int i = 0; i < N; i ++)
      tempGround[ix*10 + i] = (float) ix  + 0.777; 
