...
void  seq_heat_dist(float * playground, unsigned int N, unsigned int iterations)
{
  ...
  for( k = 0; k < iterations; k++)
  {
   ...
   for(int i = 0; i < N*N; i++) {
      cout<<"i-> "<<i/N<<" j->"<<i % N<< " value->"<<  playground[i]<<endl;
   }
...
__global__ void testLoop(float * tempGround, float * playground, int intN, int iterations);
...
void  gpu_heat_dist(float * playground, unsigned int N, unsigned int iterations)
{
   ...
   testLoop<<<blockNum, threadNum >>>(d_temp, d_playground, N, iterations);

   cudaMemcpy(h_playgroundResult, d_playground, groundSize,  cudaMemcpyDeviceToHost);
   cudaMemcpy(h_tempResult, d_temp, groundSize,  cudaMemcpyDeviceToHost);

   for(int i = 0; i < N*N; i++) {
      // cout<<"i-> "<<i/N<<" j->"<<i % N<< " value->"<<  h_playgroundResult[i]<<endl;
      cout<<"i-> "<<i/N<<" j->"<<i % N<< " value->"<<  h_tempResult[i]<<endl;
   }
...
__global__ void testLoop(float * tempGround, float * playground, int intN, int iterations) {
   ...
   tempGround[iy] = playground[iy];

   if(ix<intN ) {
      for(int i = 1; i < intN -1 ; i++) {
         if ((ix > 0) && (ix < intN -1))   {
            tempGround[index(ix, i, intN)] = (
               playground[index(ix - 1, i, intN)] +
               playground[index(ix + 1, i, intN)] +
               playground[index(ix, i - 1, intN)] +
               playground[index(ix, i + 1, intN)] )/4.0;   

            // tempGround[index(ix, i, intN)] = playground[index(ix, i, intN)];         
            // tempGround[index(ix, i, intN)] = 0.123;         
        }    
      }
   }
   // playground[ix] = tempGround[ix];      
